...
public class PdfExportService {
  ...
  public Path exportSideBySide(String title,
    ...
    try (OutputStream os = Files.newOutputStream(out)) {
      ...
      Paragraph pTitle = new Paragraph("Notes Export — " + safeTitle, h1);
      pTitle.setSpacingAfter(6f);
      doc.add(pTitle);
      doc.add(new Paragraph("Class: " + (className==null?"—":className), meta));
      doc.add(new Paragraph("Recorded: " + (recordedAt==null?"—":recordedAt), meta));
